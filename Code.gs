 ...
 *
 * 컬럼: A=Contract | B=이름 | C=티커 | D=Lore | E=MC | F=FDV | G=24h거래량
 *       H=홀더 | I=유동성 | J=Token Age | K=체인 | L=Buy Tax | M=Sell Tax | N=DEX Tax | O=조회 시각
 */
...
function setupHeaders() {
  const sheet = SpreadsheetApp.getActiveSheet();
  const h = ["Contract Address","프로젝트 이름","토큰 티커","Lore","MC ($)","FDV ($)",
